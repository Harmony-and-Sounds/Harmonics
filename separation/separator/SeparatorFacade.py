...
def createSeparation(filename,project):
    ...
    for voice in voices:
        instrument = voice.instrument
        audio_to_midi_melodia(infile=f'{userPath}/{instrument}/{instrument}.mp3',outfile=f'{path}/{instrument}_midi.mid',bpm=160)
        scoreWriter.createScoreFromMidi(f'{path}/{instrument}_midi.mid',f'{userPath}/{instrument}/{instrument}_midi.mid',path,instrument)
